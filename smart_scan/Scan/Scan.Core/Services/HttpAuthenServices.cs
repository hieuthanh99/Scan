...
{
    public class HttpAuthenServices
    {
        private static readonly ILog logger = LogManager.GetLogger(typeof(HttpAuthenServices));
        public string restBaseUrl { get; set; }
        public string tokenName { get; set; }
        public string tokenValue { get; set; }

        private static HttpAuthenServices _httpAuthenServices { get; set; }
        public static HttpAuthenServices Instance()
        {
            if (_httpAuthenServices == null)
            {
                _httpAuthenServices = new HttpAuthenServices();
            }

            _httpAuthenServices.restBaseUrl = AppConfig.AuthAPI;
            _httpAuthenServices.tokenName = ConfigurationManager.AppSettings["TokenName"].ToString();
            _httpAuthenServices.tokenValue = (IoC.Get<ApplicationViewModel>().CurrentUser ?? new UserInfo()).Token;

            return _httpAuthenServices;
        }

        public HttpAuthenServices()
        {
            restBaseUrl = AppConfig.AuthAPI;
