...
export class FormRegexDirective {
  ...
  @HostListener('paste', ['$event']) onPaste(event: KeyboardEvent) {
    if (!!this.regex) {
      this._el.nativeElement.value = turnAlphanumberic(this._el.nativeElement.value, this.regex);
      this.control?.control?.setValue(turnAlphanumberic(this._el.nativeElement.value, this.regex));
    }
  }
